...
def test_read_emb(ent_emb, train_ill, test_ill, bs=128, candidate_topk=50):
    ...
    hit1, hit50 = test_topk_res(index)
    with open(ResultFile, 'a') as f:
        f.write('seed number:' + str(len(train_ill)))
        f.write('\n')
        f.write('train set hit 1 : ' + str(hit1))
        f.write('    hit 50 : ' + str(hit50))
        f.write('\n')
    ...
    hit1, hit50 = test_topk_res(index)
    with open(ResultFile, 'a') as f:
        f.write('test set hit 1 : ' + str(hit1))
        f.write('    hit 50 : ' + str(hit50))
        f.write('\n')
...
def main():
    ...
    # neigh_dict(key = entity ,value = (padding) neighbors of entity),随机选50个
    ...
    entity_pairs_1, neighViewInterF = neighborView_interaction_F_gene(entity_pairs, ent_emb, neigh_dict, ent_pad_id,
                                                                      kernel_num=KERNEL_NUM, cuda_num=cuda_num,
                                                                      batch_size=2048)
